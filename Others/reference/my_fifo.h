// my_fifo.h
//   copyright Derek Chiou

// this file contains fifos of far more power than is necessary for the MEMOCODE design contest.
...
template <class T> class my_fifo_t {
 public:
  ...
  my_fifo_t(const char *name, int size);
  my_fifo_t(int size);
  my_fifo_t();
  
  void init(const char *name, int size);
  void init(int size);
  void init(const char *_name, int size, T empty);
  void init(int size, T empty);
  void init_time(const char *_name, int size, T empty);
...
// same as constructor
template <class T> my_fifo_t<T>::my_fifo_t(const char *_name, int size) {
  max_size = size;
  num = 0;
  els = new T [size];
  head = tail = 0;
...
// same as constructor
template <class T> void my_fifo_t<T>::init(const char *_name, int size, T empty) {
  init(size, empty);
  name = _name;
}

// same as constructor
template <class T> void my_fifo_t<T>::init(const char *_name, int size) {
  init(size);
  name = _name;
}

// same as constructor
template <class T> void my_fifo_t<T>::init_time(const char *_name, int size, T empty) {
  name = _name;
  max_size = size;
  num = 0;
  els = new T [size];
